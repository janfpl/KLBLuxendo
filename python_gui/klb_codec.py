...
def find_hdf5_image_datasets(path: str | os.PathLike[str]) -> list[Hdf5DatasetInfo]:
...
def compress_hdf5_to_klb(
    ...
    block_size: Sequence[int] = (96, 96, 8, 1, 1),
    ...
    with h5py.File(h5_path, "r") as h5_file:
        ...
        if write_metadata_sidecar and metadata_text:
            output_path.with_suffix(output_path.suffix + ".metadata.json").write_text(metadata_text, encoding="utf-8")
...
def compress_hdf5_all_layers_to_klb_bundle(
    ...
    _write_exact_luxendo_archive(output_dir / EXACT_LUXENDO_ARCHIVE, [(h5_path, Path(h5_path.name))], cancel_check)
    manifest["exact_luxendo_archive"] = EXACT_LUXENDO_ARCHIVE
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def _compress_luxendo_file_entry(
    ...
    datasets = find_hdf5_image_datasets(h5_path)
    if not datasets:
...
def compress_directory_to_klb_bundle(
    ...
    for lux_file in lux_files:
        ...
        datasets = find_hdf5_image_datasets(lux_file)
        if not datasets:
            raise ValueError(f"No Luxendo-style Data dataset was found in {lux_file}")

        def inner_progress(_done: int, _total: int, message: str, rel_path: Path = rel) -> None:
            if progress:
                progress(completed, max(total_steps, 1), f"{rel_path}: {message}")
    ...
    if archive_other_files and (other_files or other_dirs):
        ...
        with zipfile.ZipFile(archive_path, "w", compression=zipfile.ZIP_DEFLATED, allowZip64=True) as archive:
            for other_dir in other_dirs:
                _raise_if_cancelled(cancel_check)
                archive.write(other_dir, other_dir.relative_to(source_dir).as_posix().rstrip("/") + "/")
                archived_dir_count += 1
            for other_file in other_files:
                _raise_if_cancelled(cancel_check)
                archive.write(other_file, other_file.relative_to(source_dir).as_posix())
                archived_count += 1
                completed += 1
                if progress:
                    progress(completed, max(total_steps, 1), f"Archived {other_file.relative_to(source_dir)}")
    ...
    exact_archive_path = output_dir / EXACT_LUXENDO_ARCHIVE
    _write_exact_luxendo_archive(
        exact_archive_path,
        [(lux_file, lux_file.relative_to(source_dir)) for lux_file in lux_files],
        cancel_check,
    )
    manifest["exact_luxendo_archive"] = exact_archive_path.relative_to(output_dir).as_posix()
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def repair_bundle_manifest(
    ...
    lux_entries = []
    exact_files: list[tuple[Path, Path]] = []
    for layer_dir in sorted(path for path in layers_root.rglob("*") if path.is_dir()):
        ...
        if source_file is not None and source_file.exists():
            ...
            exact_files.append((source_file, source_relative))
    ...
    if exact_files:
        exact_archive = bundle_dir / EXACT_LUXENDO_ARCHIVE
        _write_exact_luxendo_archive(exact_archive, exact_files, None)
        manifest["exact_luxendo_archive"] = exact_archive.name
        manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
